...
from boooks.framework.http import JSONResource, json_response
from boooks.api.models import User
from boooks.api.roles import PublicRole
...
def get_authenticated_user(given_role_names=None):
    ...
    token = request.headers.get('X-Boooks-Token', '')
    if not token:
        return None, forbidden('Missing X-Boooks-Token')

    user = User.from_token(token)
    if not user:
        return None, forbidden('Invalid X-Boooks-Token')
